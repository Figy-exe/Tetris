...
class ModeleTetris():
        ...
        for i in range(self.__haut):
            ...
            for j in range(self.__larg):
                if i<4:
                    ...
                    lign.append(-1)
            self.__terrain.append(lign)
        self.__forme = Forme(self)
        self.__suivant=Forme(self)
        self.__score=0
        self.__nb_changement=0
    ...
    def est_ligne_complete(self,lig):
        """ModeleTetris->bool
        retourne True si la ligne lig est complète False sinon
        """
        for i in range(self.__larg):
    ...
    def supprime_ligne(self,lig):
        """ModeleTetris->None
        supprime la ligne numéro lig
        """
        for i in range(self.__larg):
            self.__terrain[lig][i]=-1
            for j in range(lig,self.__base,-1):
                if j==self.__base:
                    self.__terrain[j][i]=-1
                else:
                    self.__terrain[j][i]=self.__terrain[j-1][i]
                    
    def supprime_lignes_completes(self):
        """ModeleTetris->None
        supprime les lignes complètes du terrain
        """
    ...
    def get_coords_suivante(self):
        ...
        """
        return self.__suivant.get_coords_relatives()
    
    def get_couleur_suivante(self):
        """ModeleTetris->int
        retourne la couleur de la forme suivante
        """
        return self.__suivant.get_couleur()
    
    def reinitialiser(self):
        """
        ModeleTetris->None
        Reinitialise le tetris
        """
        self.__terrain=[]
        for i in range(self.__haut):
            lign=[]
    ...
    def forme_change(self):
        ...
        change la forme
        """
        if self.__nb_changement < 10:
            self.__forme.change()
            self.__nb_changement+=1
...
class Forme():
    ...
    def __init__(self,mod_tetris):
        ...
        self.__x0=self.calc_x()
        self.__y0=self.calc_y()
    ...
    def get_coords_relatives(self):
        """
        Forme->list(tuple(int))
        retourne une copie de la liste des coordonnées relatives de self
        """
    ...
    def calc_x(self):
        """
        Forme->None
        retourne un x adéquat pour la forme
        """
        x_min = self.__forme[0][0]
        x_max = self.__forme[0][0]
        for i,j in self.__forme:
            if i > x_max:
                x_max = i
            if i < x_min:
                x_min = i
        return randint(-x_min,self.__modele.get_largeur()-x_max-1)
        
    def calc_y(self):
        """
        Forme->None
        retourne un y adéquat pour la forme
        """
        y_min = 0
        for i,j in self.__forme:
            if j < y_min:
    ...
    def change(self):
        """Forme->None
        change la forme pour celle suivante dans LES_FORMES
        """
        col_prec=self.__couleur
        forme_prec=self.__forme
        self.__couleur=(self.__couleur+1)%7
        self.__forme=LES_FORMES[self.__couleur]
        if not self.position_valide():
            self.__couleur=col_prec
            self.__forme=forme_prec
