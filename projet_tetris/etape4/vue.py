...
class VueTetris():
    ...
    def __init__(self,mod_tetris):
        ...
        self.__btn_pause = tkinter.Button(fram, text="Commencer",command=self.change_pause)
        self.__btn_pause.pack()
        self.__pause=True
    ...
    def dessinne_case_suivante(self,x,y,coul):
        ...
        """
        self.__can_fsuivante.itemconfigure(self.__les_suivant[i][j],fill=COULEURS[col])
    ...
    def nettoie_forme_suivante(self):
        ...
        for i in range(SUIVANT):
            for j in range(SUIVANT):
                self.__can_fsuivante.itemconfigure(self.__les_suivant[i][j],fill=COULEURS[-1])
    ...
    def change_pause(self):
        """
        VueTetris->None
        change le paramètre pause pour remettre en route le tetris
        où au contraire le mettre en pause
        change aussi le texte du boutton en fonction
        et si besoin reinitialise le tetris
        """
        self.__pause=not self.__pause
        if self.__btn_pause["text"]=="Recommencer":
            self.__modele.reinitialiser()
            self.__pause=not self.__pause
        if self.__pause:
            self.btn_reprendre()
        else:
    ...
    def btn_pause(self):
        """
        VueTetris->None
        Change le texte du __btn_pause pour Pause
        """
        self.__btn_pause["text"]="Pause"
    
    def btn_recommencer(self):
        """
        VueTetris->None
        Change le texte du __btn_pause pour Recommencer
        """
        self.__btn_pause["text"]="Recommencer"
    
    
    def btn_reprendre(self):
        """
        VueTetris->None
        Change le texte du __btn_pause pour Reprendre
        """
        self.__btn_pause["text"]="Reprendre"